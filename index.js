...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5588981321692\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiin @${num.split('@')[0]}\nBem vindo ao grupo shitmerda *${mdata.subject}* Leia a descrição por favor 😊`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5588981321692@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5588981321692@s.whatsapp.net'
                        /******ApiKey Input******/
